...
class Ridge:
    ...
    def predict(self, X, Y, test_x):
        ...
        ridge_matrix = torch.matmul(XT, X) + self.sigma2 * torch.eye(n_dims).to(X.get_device())  # batch_size x n_dims x n_dims
...
class DiscreteMMSE:
    def __init__(self, task_pool, sigma2):
    ...
    def predict(self, X, Y, test_x, W):
        ...
        alpha = -torch.sum((Y - torch.matmul(X, W))**2, axis=1) / (2*self.sigma2)
